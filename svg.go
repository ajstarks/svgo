...
func New(w io.Writer) *SVG { return &SVG{w, false} }
...
// prolog writes the xml prlog, that precedes the single top level <svg ...> entity
func  (svg *SVG) prolog() {
	if !svg.prologDone {
		svg.print(svgprolog)
		svg.prologDone = true
...
// ExternalStyleSheet *must* precede calls to Startxxx(),
// if ExternalStyleSheet is called after Start, it will have no effect.
// cssfile is the name of the file containing the style sheet
// Standard Reference: http://www.w3.org/1999/06/REC-xml-stylesheet-19990629/
// from: 6.9 http://www.w3.org/TR/SVG/styling.html#ReferencingExternalStyleSheets
func (svg *SVG) ExternalStyleSheet(cssfile string) {
	svg.prolog()
	svg.printf(`<?xml-stylesheet type="text/css" href="%s"?>`+"\n", cssfile)
}
...
// StyleStart embeds CSS style sheet within an SVG style entity the style entity
// contains CDATA to protect CSS style content from xml interpretation.
// StyleStart begins a style entity, StyleRule or StyleStrings adds the style data, 
// then StyleEnd completes the Style entity
// Standard entity reference: http://www.w3.org/TR/SVG/styling.html#StyleElement
// Using CSS within SVG reference: http://www.w3.org/TR/SVG/styling.html#StylingWithCSS
func (svg *SVG) StyleStart(attributes ...string) {
	svg.print(`<style type="text/css"`)
	switch {
...
// StyleRule adds a CSS style sheet rule within the style entity
// StyleRule adds one CSS rule
// example rule: `selector` { `declaration[0]` `declaration[1]` ... }
func (svg *SVG) StyleRule(selector string, delaration...string) {
	svg.printf(`%s { `, selector)
	for _, d := range delaration {
		svg.print(d, ` `)
	}
	svg.print("}\n")
}

// StyleStrings adds Style sheet strings within an SVG style entity
// StyleStrings can be called more than once between a StyleStart and StyleEnd
// data could be created using, e.g. svg.RGB(...), fmt.Sprintf() or literal strings
func (svg *SVG) StyleStrings(data ...string) {
	for _, v := range data {
		svg.println(v)
	}
}

// StyleEnd completes, closes, the CDATA and SVG Style entity
func (svg *SVG) StyleEnd() {
	svg.printf("]]>\n</style>\n")
}
